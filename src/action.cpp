...
Action::Action(const Action &other) : _event(other._event), _type(other._type) {}
...
Action::Action(const sf::Keyboard::Key &key, int type)
    : _type(type), _event(sf::Event::KeyPressed{.code = key}) {}
...
bool Action::test() const {
  if (_event.is<sf::Event::KeyPressed>()) {
    return sf::Event::KeyPressed().code == _event.getIf<sf::Event::KeyPressed>()->code;
  }

  if (_event.is<sf::Event::MouseButtonPressed>()) {
    return sf::Event::MouseButtonPressed().button ==
           _event.getIf<sf::Event::MouseButtonPressed>()->button;
  }
