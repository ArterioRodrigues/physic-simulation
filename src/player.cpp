...
void Player::update(sf::Time deltaTime) {
  ...
  if (rotation != 0) {
    sf::Angle angle = sf::degrees((rotation > 0 ? 1 : -1) * TURNING_ANGLE * seconds);
    _shape.rotate(angle);
  }
  if (isMoving) {
...
void Player::draw(sf::RenderTarget &target, sf::RenderStates states) const { target.draw(_shape, states); }
